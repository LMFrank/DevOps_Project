...
init_database() {
  ...
  echo "create user zabbix@localhost identified by 'zabbix@123';" | mariadb -uroot
  ...
  # 导入初始化数据
  zcat /usr/share/zabbix-sql-scripts/mysql/server.sql.gz | mariadb --default-character-set=utf8mb4 -uzabbix -pzabbix@123 zabbix
  sed -i 's/# DBPassword=/DBPassword=zabbix@123/g' /etc/zabbix/zabbix_server.conf
